...
public class OptionShot : MonoBehaviour
{
    ...
    public void Explode(GameObject fractureGameObject, float percentage)
    {
        ...
        // StartCoroutine(DestroyChunksCoroutine(chunksToRemove, chunksToDestroy));
        for (int i = 0; i < chunksToDestroy; i++)
        {
            ...
            chunksToRemove[i].gameObject.SetActive(false);
            ;
        }
    }
    
    IEnumerator DestroyChunksCoroutine(Rigidbody[] chunks, int chunksToDestroy)
    {
        for (int i = 0; i < chunksToDestroy; i++)
        {
            // add a force to the chunk
    ...
    public IEnumerator LeaveScreenWithDelay(float waitTime)
    {
    ...
    public void LeaveScreen(float speed = 1f, float height = 2.2f)
    {
        shootable = false;
