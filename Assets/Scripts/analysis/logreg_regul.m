...
for irun = 1:nrun
    ...
    if nrun == 1
        % set initialization value to zero
        b_ini = zeros(nreg,1);
    else
        if ~isempty(b_sigma)
            % sample initialization values from prior distribution
            b_ini = normrnd(zeros(nreg,1),b_sigma);
        else
            % sample initialization values uniformly in [-10,+10]
    ...
    if fit_e
        if nrun == 1
            ...
            if ~isempty(e_betab)
                ...
                % sample initialization values uniformly in [0,1]
                e_ini = unifrnd(0,1);
            end
        end
        p_ini = [b_ini;e_ini];
    ...
    % fit logistic regression model using fmincon
    [pval(irun,:),fval(irun)] = ...
        fmincon(@(pv)-get_ll(pv),p_ini,[],[],[],[],p_min,p_max,[],options);
end

% get best fit among random starting points
[~,irun] = min(fval);
b = pval(irun,1:nreg);
if fit_e
    e = pval(irun,1+nreg);
